...
class TrainingModel(object):
    ...
    def loop_train(self, train_tuple, valid_tuple, test_tuple):
        ...
        filename =\
                'model_{}_task_{}_nhids_{}_embsize_{}_drate_{}_prate_{}_nfilter_{}_bs_{}_lr_{}_mark_{}_nsample_{}'.format(self.kwargs['model'],
                        self.kwargs['task'], self.kwargs['n_hids'], self.kwargs['embsize'],kwargs['drate'] ,
                        kwargs['prate'] ,kwargs['n_filter'], kwargs['batch_size'], kwargs['lr'] ,kwargs['mark'],self.kwargs['n_sample'])
        self.save_folder = join(dirname(__file__), filename+time.strftime('-%b%d.%Hh%M',time.localtime()))
        self.recorder = MyRecorder(savefolder = self.save_folder)

        if not os.path.exists(self.save_folder):
            os.mkdir(self.save_folder)
        with open(join(self.save_folder, 'ModelArgs.json'), 'w') as f:
            json.dump(self.kwargs, f)
        with open(join(self.save_folder,'vocab.pkl'), 'wb') as f:
            pickle.dump(self.vocab,f)

        self.logg = mylog(self.save_folder)
        self.logg.logging(filename)
        self.logg.logging('Start training, The number of samples is {}'.format(len(train_tuple[0])))
        self.logg.logging('Start training, The number of vocab is {}'.format(len(self.vocab)))

        #train_data = self.gen_batches(train_tuple)
        #val_data = self.gen_batches(valid_tuple)

        train_data = self.create_buckets_batches(train_tuple[0], train_tuple[1], self.vocab['<mask>'])
        val_data = self.create_buckets_batches(valid_tuple[0], valid_tuple[1], self.vocab['<mask>'])
        test_data = self.create_buckets_batches(test_tuple[0], test_tuple[1], self.vocab['<mask>'])
        print 'data size'
        ...
        for self.epoch in range(1, args.epochs+1):
            ...
            if not self.eval:
                ...
                # self.training(train_data, test_data)
                train_loss, train_acc, train_reward ,res= self.validation(train_data)
                # print(torch.mean(self.model.embedding_layer.weight))
                self.logg.logging('| epoch {:3d} | time: {:5.2f}s |'\
                    'loss {:5.3f} | reward {:5.3} | acc {:5.5}|'.format(
                            self.epoch, (time.time() - epoch_start_time),train_loss, train_reward,\
                                    train_acc))

            val_loss, acc, reward, acc_vec = self.validation(val_data)
            self.logg.logging('| epoch {:3d} | time: {:5.2f}s |'\
                    'loss {:5.3f} | reward {:5.3} | acc {:5.5}|'.format(
                            self.epoch, (time.time() - epoch_start_time),val_loss, reward,\
                                    acc))

            test_loss, test_acc, test_reward, test_acc_vec = self.validation(test_data)
            self.logg.logging('| Testing | time: {:5.2f}s |'\
                    'loss {:5.3f} | reward {:5.3} | acc {:5.5}|'.format( (time.time() -
                        epoch_start_time),test_loss, test_reward,test_acc))
            ...
            if not best_val_reward or acc > best_val_reward:
                filename = join(self.save_folder,
                       "ep%d_model_reward%.4f.pkl"%(self.epoch, acc))
                       # "ep%d-min-mean-valid-reward%.4f.pkl" % (self.epoch, reward))
                with open(filename, 'wb') as f:
                    torch.save(self.model.state_dict(), f)
                    unchange = 0
                    best_val_reward = acc
                    best_acc_vec = acc_vec
                    self.logg.logging(filename)
                    self.logg.logging('----------------save a new  best model-------------------------')
                    test_accu = test_acc
                # test_loss, test_acc, test_reward, test_acc_vec = self.validation(test_data)
                # self.logg.logging('| Testing | time: {:5.2f}s |'\
                #         'loss {:5.3f} | reward {:5.3} | acc {:5.3}|'.format( (time.time() -
                #             epoch_start_time),test_loss, test_reward,test_acc))

            # if unchange > 30:
            #     # Anneal the learning rate if no improvement has been seen in the validation dataset.
            #     filename = join(self.save_folder,
            #            "ep%d-bak-reward%.4f.pkl" % (self.epoch, reward))
            #     with open(filename, 'wb') as f:
            ...
            #         self.optimizer = optim.SGD(self.model.parameters(), lr = self.lr)
            #     # self.prate /= 1.2
            #     # self.model.set_prate(self.prate)
            #     self.logg.logging('----------------update lr-------------------------')
            #     unchange = 0
            # print best_acc_vec.size()
            if unchange >80 or self.epoch >400:
                break
    ...
    def train(self,train_data):
        ...
        for i in range(n_batches):
            ...
            loss_t, acc_t, reward_t , lensss , res= self.model(data, targets1)
